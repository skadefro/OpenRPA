...
namespace OpenRPA.Core.Image
{
    public class Util
    {
        ...
        {
            SaveImageStamped(img, Extensions.MyPictures, message);
        }
        ...
        public static async Task<Bitmap> LoadWorkflowImage(string basepath, string id)
        {
            try
            {
                if (string.IsNullOrEmpty(basepath)) { basepath = Core.Extensions.ProjectsDirectory; }
